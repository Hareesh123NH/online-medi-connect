...
const getDoctorRegisrations = async (req, res) => {

  try {

    const tempDoctors = await TempDoctor.find();

    res.status(200).json(tempDoctors);
  } catch (error) {
    console.error("Error fetch doctors:", error);
    res.status(500).json({ message: "Server error" });
  }
}


module.exports = { signupAdmin, approveDoctor, rejectDoctor, getDoctorRegisrations };