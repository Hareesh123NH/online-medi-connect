...
//get: View all accepted appointments for a patient
...
//get: View all rejected appointments for a patient
    ...
    try {
        ...
        if (page > Math.ceil(totalDoctors / limit)) {
            return res.status(200).json({
                message: "No doctors found matching the given page",
            });
        }
        ...
        const doctors = await Doctor.find(filter)
            .select("name specialization email phone createdAt")
            .skip(skip)
            .limit(limit)
            .sort({ createdAt: -1 });
